...
void BuildRenderPassesForFrame(RendererContext* renderer_context, RecordContext* record_context, WorldEntitySystem* world_system, AssetEntitySystem* asset_system, u64 world_entity_guid) {
	...
	*atmosphere_parameters_cpu_address = atmosphere_parameters;
	...
	UpdateMeshletPageTableRenderPass{ world_system, asset_system, &renderer_world.meshlet_streaming_feedback_queue, world_entity.meshlet_streaming_system.data, renderer_context->async_transfer_queue }.RecordPass(record_context);
	...
	if (renderer_world.debug_freeze_culling_camera.enabled == false) {
		...
		MeshletCullingRenderPass{ world_system, &renderer_world.meshlet_streaming_feedback_queue, MeshletCullingPass::Disocclusion }.RecordPass(record_context);
