...
compile_const u32 max_transient_resource_table_entries = 16 * 1024;
...
RendererContext* CreateRendererContext(StackAllocator* alloc) {
	...
	context->mesh_asset_buffer_address = GetBufferGpuVirtualAddress(context->mesh_asset_buffer);
	
	context->meshlet_rtas_buffer         = CreateBufferResource(graphics_context, meshlet_rtas_buffer_size, CreateResourceFlags::UAV | CreateResourceFlags::RTAS);
