...
inline void BitArraySetBit(ArrayView<u64> mask, u32 index) {
	mask[index / 64u] |= 1ull << (index % 64u);
}

inline bool BitArrayTestBit(ArrayView<u64> mask, u32 index) {
...
inline void BitArraySetBitRange(ArrayView<u64> mask, u32 offset, u32 count) {
	...
	u32 qword_index_0 = offset / 64u;
	u32 qword_index_1 = (offset + count - 1) / 64u;
	...
	if (qword_index_0 == qword_index_1) {
		...
		for (u32 i = qword_index_0 + 1; i < qword_index_1; i += 1) {
			mask[i] = u64_max;
		}
