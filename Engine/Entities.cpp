...
static void UpdateMeshEntityGpuComponents(StackAllocator* alloc, RecordContext* record_context, WorldEntitySystem& world_system, AssetEntitySystem& asset_system, Array<GpuComponentUploadBuffer>& gpu_uploads) {
	ProfilerScope("UpdateMeshEntityGpuComponents");
	
	auto mesh_asset_streams = ExtractComponentStreams<MeshAssetType>(QueryEntityTypeArray<MeshAssetType>(asset_system));
	
	auto* entity_array = QueryEntityTypeArray<MeshEntityType>(world_system);
	...
	for (u64 qword_index = 0; qword_index < dirty_mask.count; qword_index += 1) {
		u64 dirty_qword = dirty_mask[qword_index];
		dirty_count += CountSetBits(dirty_qword);
		
		u64 dirty_or_prev_dirty_qword = dirty_qword | prev_dirty_mask[qword_index];
...
static void UpdateLightEntityGpuComponents(StackAllocator* alloc, RecordContext* record_context, WorldEntitySystem& world_system, Array<GpuComponentUploadBuffer>& gpu_uploads) {
	ProfilerScope("UpdateLightEntityGpuComponents");
	
	auto* entity_array = QueryEntityTypeArray<LightEntityType>(world_system);
