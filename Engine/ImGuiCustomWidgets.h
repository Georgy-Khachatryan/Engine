...
namespace ImGui {
	...
	bool TableCollapsingHeader(const char* label, ImGuiTreeNodeFlags flags = 0);
	void TableLabelText(const char* label, float max_width = -1.f); // Right justified, clips with ellipsis on the right side.
	...
	bool TableEntityComboBox(StackAllocator* alloc, const char* label, EntitySystemBase* entity_system, u64* guid, EntityTypeID entity_type_id);
	bool TableEntityComboBoxWithColor(StackAllocator* alloc, const char* label, EntitySystemBase* entity_system, float* color, u32 channel_count, u64* guid, EntityTypeID entity_type_id);
